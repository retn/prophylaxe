...
public class ControllerData {
	...
	private List<Distraction> fetchDistractions(int i2) {
		...
		List<String> emotionids=tables.query("spl_distraction", "fk_distraction_emotion1"); //?
		List<Distraction> distractions=new ArrayList<Distraction>();
		for(int i=0;i<ids.size();i++){
			if(i==i2){
				Distraction distraction=new Distraction(Integer.parseInt(ids.get(i)),Integer.parseInt(emotionids.get(i)));
	...
	private List<HelpPerson> fetchHelpPeople() {
		...
		List<String> phone_numbers=tables.query("spl_ec_help_person", "phone_number");
		List<HelpPerson> helpPersons= new ArrayList<HelpPerson>();
		for(int i=0;i<ids.size();i++){
			HelpPerson person=new HelpPerson(Integer.parseInt(ids.get(i)),names.get(i),phone_numbers.get(i));
	...
	private List<DiaryEntry> fetchDiaryEntries() {
		...
		for(int i=0;i<ids.size();i++){
			DiaryEntry entry;
			try {
				entry = new DiaryEntry(Integer.parseInt(ids.get(i)),titles.get(i),contens.get(i),new SimpleDateFormat("MMMM d, yyyy",Locale.ENGLISH).parse(createds.get(i)),0,null);
	...
	private void fetchDiaryEntriesPicture(List<DiaryEntry> diaryEntries2) {
		...
		List<String> entryids=tables.query("spl_diary_entry_has_picture", "entryID");
		
		for(DiaryEntry entry:diaryEntries2){
			for(int i=0;i<entryids.size();i++){
				if(entry.getId()==Integer.parseInt(entryids.get(i))){
					Media media=new Media(Integer.parseInt(ids.get(i)),entry.getId(),Type.Image);
	...
	private void fetchDiaryEntriesMood(List<DiaryEntry> diaryEntries2) {
		List<String> entryids=tables.query("spl_diary_entry_has_mood", "entryID");
		List<String> emotionids=tables.query("spl_diary_entry_has_mood", "emotionID");
